...
exports.resolveReminder = resolveReminder;
exports.generateRemindersFromMeta = generateRemindersFromMeta;
const db_1 = require("../../config/db");
async function listReminders() {
    const r = await db_1.client.query(`
...
async function resolveReminder(id) {
    await db_1.client.query('UPDATE reminders SET resolved = true WHERE id = $1', [id]);
}
async function generateRemindersFromMeta() {
    console.log('🔔 Generating reminders...');
    // Get current user email from OAuth (simplified - you may need to adjust this)
    const userEmailQuery = await db_1.client.query('SELECT scope FROM oauth_tokens ORDER BY id DESC LIMIT 1');
    ...
    const unreadImportant = await db_1.client.query(`
  ...
  `);
    for (const email of unreadImportant.rows) {
        ...
        const exists = await db_1.client.query('SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false', [email.id, reason]);
        if (exists.rowCount === 0) {
            await db_1.client.query('INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)', [email.id, reminderText, reason, email.is_hierarchy ? 8 : 7]);
    ...
    const missingMoM = await db_1.client.query(`
  ...
  `);
    for (const meeting of missingMoM.rows) {
        ...
        const exists = await db_1.client.query('SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false', [meeting.id, 'missing_mom']);
        if (exists.rowCount === 0) {
            await db_1.client.query('INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)', [meeting.id, reminderText, 'missing_mom', 8]);
            console.log(`✓ Created reminder for missing MoM: ${meeting.id}`);
        }
    }
    // 3. ESCALATION EMAILS
    console.log('Checking escalation emails...');
    const escalations = await db_1.client.query(`
  ...
  `);
    for (const esc of escalations.rows) {
        const exists = await db_1.client.query('SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false', [esc.id, 'escalation']);
        if (exists.rowCount === 0) {
            await db_1.client.query('INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)', [esc.id, `Escalation detected from ${esc.sender_email}: "${esc.subject}"`, 'escalation', 10]);
            console.log(`✓ Created reminder for escalation: ${esc.id}`);
        }
    }
    // 4. URGENT CLIENT REQUESTS
    console.log('Checking urgent client requests...');
    const urgentClients = await db_1.client.query(`
    SELECT e.id, e.subject, e.sender_email, e.is_unread
    FROM emails e
    JOIN email_meta m ON m.email_id = e.id
  ...
  `);
    for (const urgent of urgentClients.rows) {
        const exists = await db_1.client.query('SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false', [urgent.id, 'urgent_client']);
        if (exists.rowCount === 0) {
            await db_1.client.query('INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)', [urgent.id, `Urgent request from client ${urgent.sender_email}: "${urgent.subject}"`, 'urgent_client', 9]);
