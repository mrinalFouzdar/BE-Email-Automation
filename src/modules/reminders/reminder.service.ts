...
export async function generateRemindersFromMeta() {
  ...
  for (const email of unreadImportant.rows) {
    ...
    const exists = await client.query(
      'SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false',
      [email.id, reason]
    );

    if (exists.rowCount === 0) {
      await client.query(
        'INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)',
        [email.id, reminderText, reason, email.is_hierarchy ? 8 : 7]
      );
  ...
  for (const meeting of missingMoM.rows) {
    ...
    const exists = await client.query(
      'SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false',
      [meeting.id, 'missing_mom']
    );

    if (exists.rowCount === 0) {
      await client.query(
        'INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)',
        [meeting.id, reminderText, 'missing_mom', 8]
      );
  ...
  for (const esc of escalations.rows) {
    ...
    if (exists.rowCount === 0) {
      await client.query(
        'INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)',
        [esc.id, `Escalation detected from ${esc.sender_email}: "${esc.subject}"`, 'escalation', 10]
      );
  ...
  for (const urgent of urgentClients.rows) {
    const exists = await client.query(
      'SELECT 1 FROM reminders WHERE email_id = $1 AND reason = $2 AND resolved = false',
      [urgent.id, 'urgent_client']
    );

    if (exists.rowCount === 0) {
      await client.query(
        'INSERT INTO reminders(email_id, reminder_text, reason, priority) VALUES($1,$2,$3,$4)',
        [urgent.id, `Urgent request from client ${urgent.sender_email}: "${urgent.subject}"`, 'urgent_client', 9]
      );
