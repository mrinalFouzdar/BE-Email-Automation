...
export class LabelService {
   ...
   */
  async initializeSystemLabels(): Promise<void> {
    const systemLabels = [
    ...
    for (const label of systemLabels) {
      await client.query(
        `INSERT INTO labels (name, color, description, is_system)
         VALUES ($1, $2, $3, true)
         ON CONFLICT (id) DO NOTHING`, // Note: name is not unique constraint, so we check existence first
        [label.name, label.color, label.description]
      );
      
      // Better check to avoid duplicates if ID not known
      const exists = await client.query('SELECT id FROM labels WHERE name = $1 AND is_system = true', [label.name]);
   ...
   */
  async findLabelByName(name: string, userId?: number): Promise<Label | null> {
