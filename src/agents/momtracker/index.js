...
function subjectsAreSimilar(subject1, subject2) {
    // Remove common prefixes like Re:, Fwd:, etc.
    const clean = (s) => s.replace(/^(re:|fwd:|fw:)\s*/gi, '').toLowerCase().trim();
...
async function main() {
  ...
  `);
    ...
    for (const e of allEmails.rows) {
        ...
        if (isMoM) {
            await client.query(`UPDATE email_meta SET is_mom = true WHERE email_id = $1`, [e.id]);
  ...
  `);
    ...
    `, [meeting.received_at, meeting.thread_id]);
        ...
        if (momFound) {
            ...
            await client.query(`UPDATE email_meta SET has_mom_received = true, related_meeting_id = $2 WHERE email_id = $1`, [meeting.id, momFound.id]);
            await client.query(`UPDATE email_meta SET related_meeting_id = $2 WHERE email_id = $1`, [momFound.id, meeting.id]);
            ...
            await client.query(`UPDATE email_meta SET has_mom_received = false WHERE email_id = $1`, [meeting.id]);
